...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname 114a) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp")) #f)))
; A UFO is a Posn. 
...
; Any -> Boolean
;  is an element of the tank collection
(check-expect (TANK? "bob") #false)
(check-expect (TANK? (make-tank "b" 21)) #false)
(check-expect (TANK? (make-tank 32 "b")) #false)
(check-expect (TANK? (make-tank "B" "B")) #false)
(check-expect (TANK? (make-tank 1 2)) #true)
(define (TANK? t)
  (cond
    [(tank? t) (cond
                 [(and (number? (tank-loc t)) (number? (tank-vel t))) #true]
                 [else #false])]
    [else #false]))

; Any -> Boolean
;  is an element of the UFO collection
(check-expect (UFO? "bob") #false)
(check-expect (UFO? (make-posn "b" 2)) #false)
(check-expect (UFO? (make-posn 32 "b")) #false)
(check-expect (UFO? (make-posn "b" "b")) #false)
(check-expect (UFO? (make-posn 1 2)) #true)
(define (UFO? u)
  (cond
    [(posn? u) (cond
                 [(and (number? (posn-x u)) (number? (posn-y u))) #true]
                 [else #false])]
    [else #false]))
...
(check-expect (SIGS? (make-sigs (make-posn 1 2) (make-tank 1 2) (make-posn 1 2))) #true)
(check-expect (SIGS? (make-sigs (make-posn 1 2) (make-tank 1 2) #false)) #true)
(check-expect (SIGS? (make-sigs (make-posn 1 2) (make-tank 1 2) #true)) #false)
(check-expect (SIGS? (make-sigs (make-posn 1 2) (make-tank "b" 2) #false)) #false)
(define (SIGS? s)
  (cond
    [(sigs? s)
     (cond
       [(and (UFO? (sigs-ufo s))
             (TANK? (sigs-tank s))
             (missile-or-not? (sigs-missile s))) #true]
       [else #false])]
    [else #false]))
