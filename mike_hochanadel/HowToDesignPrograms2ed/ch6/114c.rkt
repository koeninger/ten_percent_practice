...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname 114c) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp")) #f)))
...
; Any -> Boolean
;  is an element of the editor collection
(check-expect (EDITOR? 2) #false)
(check-expect (EDITOR? "bob") #false)
(check-expect (EDITOR? (make-editor 1 2)) #false)
(check-expect (EDITOR? (make-editor "b" 2)) #false)
(check-expect (EDITOR? (make-editor 1 "b")) #false)
(check-expect (EDITOR? (make-editor "B" "c")) #true)
(define (EDITOR? e)
  (cond
    [(editor? e) (cond
...
; Editor -> number
; editor-text-size returns the size of the editor with respect to the canvas size
;  and text size
(check-expect
 (editor-text-size (make-editor "hello world" ""))
 (image-width (text "hello world" EDITOR_TEXT_SIZE "black")))
(check-expect
 (editor-text-size (make-editor "bob " "smith"))
 (image-width (text "bob smith" EDITOR_TEXT_SIZE "black")))
(check-expect
 (editor-text-size (make-editor "" ""))
 (image-width (text "" EDITOR_TEXT_SIZE "black")))
(check-expect
 (editor-text-size (make-editor "bob " ""))
 (image-width (text "bob " EDITOR_TEXT_SIZE "black")))
(check-expect
 (editor-text-size (make-editor "" "smith"))
 (image-width (text "smith" EDITOR_TEXT_SIZE "black")))
(define (editor-text-size ed)
  (image-width (text (string-append (editor-pre ed) (editor-post ed)) EDITOR_TEXT_SIZE "black")))
