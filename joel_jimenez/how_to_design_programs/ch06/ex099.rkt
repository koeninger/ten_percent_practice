...
; Number -> Number
(check-random (random-dx 0) (random UFO-DX))
(check-random (random-dx MAX-WIDTH) (* (random UFO-DX) -1))
(check-random (random-dx (+ UFO-DX 1)) (* UFO-DX (- (random 3) 1)))
(define (random-dx x)
  (cond
    [(< (- x UFO-DX) 0) (random UFO-DX)]
    [(> (+ x UFO-DX) MAX-WIDTH) (* (random UFO-DX) -1)]
    [else (* UFO-DX (- (random 3) 1))]))
