...
class App extends Component {
	...
	onSubmit = async (event) => {
		...
		this.setState({message: 'Proccessing transaction...'});
	...
	onClick = async (event) => {
		const accounts = await web3.eth.getAccounts();
		this.setState({
			message: 'Randomly selecting a winner...'
		});
  ...
  render() {
    return (
      <div className="App">
        ...
        <p className="App-intro">This contract is managed by {this.state.manager}</p>
