...
; given two Editors, if the first one's rendered text width exceeds BOX width,
; return second Editor. Otherwise return first Editor.
; chars have different width, so counting chars is not enough.
(check-expect (check-width
               (make-editor "aa" "b")
               (make-editor "a" "b"))
              (make-editor "aa" "b"))
(check-expect (check-width
               (make-editor "zzzzzzzzzzzzzzzzzzzzzzzzo" "")
               (make-editor "zzzzzzzzzzzzzzzzzzzzzzzz" ""))
              (make-editor "zzzzzzzzzzzzzzzzzzzzzzzz" ""))
(check-expect (check-width
               (make-editor "zzzzzzzzzzzzzzzzzzzzzzzzf" "")
               (make-editor "zzzzzzzzzzzzzzzzzzzzzzzz" ""))
              (make-editor "zzzzzzzzzzzzzzzzzzzzzzzzf" ""))
(define (check-width new-ed old-ed)
  (if (<
       (image-width (beside
                      (text (editor-pre new-ed) TEXT-SIZE TEXT-COLOR)
                      CURSOR
                      (text (editor-post new-ed) TEXT-SIZE TEXT-COLOR)))
       (image-width BOX))
      new-ed
      old-ed))

(define (editor-correct? e)
  (and (editor? e) (and (string? (editor-pre e)) (string? (editor-post e)))))

(define (run pre)
  (big-bang (make-editor pre "")
    [to-draw render]
    [check-with editor-correct?]
    [on-key edit]))