...
foreach($instructions as $inst){
    $parsed = preg_split('/\s+/',$inst);
    $register = $parsed[0];
    if(!isset($registers[$register])){
        $registers[$register] = 0;
    }
    $check_reg = $parsed[4];
    if(!isset($registers[$check_reg])){
        $registers[$check_reg] = 0;
    }
    $parsed[2] = intval($parsed[2]);
    $parsed[6] = intval($parsed[6]);
    $run = false;
    switch($parsed[5]){
        case '>':
            $run = $registers[$check_reg] > $parsed[6];
            break;
        case '<':
            $run = $registers[$check_reg] < $parsed[6];
            break;
...
$max = max($registers);
$reg = array_search($max, $registers);
