...
foreach($lines as $line){
    ...
    $prog = [];
    $prog['name'] = $names[] = $info[0];
    $prog['weight'] = intval(str_replace('(','',str_replace(')','',$info[1])));
    if(count($info) > 2){
        $prog['holding'] = [];
        for($i = 3;$i<count($info);$i++){
            $prog['holding'][] = $ontop[] = str_replace(',','',$info[$i]);
        }
    }
    $programs[] = $prog;
}
...
$tree = [];
$root_name = array_diff($names,$ontop);

$root = array_pop($root_name);

$tree = createNode($root,$programs);

sums($tree);

findBrokenBranch($tree);

function createNode($name, $programs){
    $node = [];
    $node['name'] = $name;
    foreach($programs as $prog){
        if($prog['name'] == $name){
            $node['weight'] = $prog['weight'];
            if(isset($prog['holding']) && count($prog['holding']) > 0){
                $node['holding'] = [];
                foreach($prog['holding'] as $subprog){
                    $node['holding'][] = createNode($subprog,$programs);
                }
            }
        }
    }
...
function sums(&$program_tree){
    if(isset($program_tree['holding']) && count($program_tree['holding'])){
        $sum = 0;
        foreach($program_tree['holding'] as $id=>$tree){
            sums($program_tree['holding'][$id]);
            $sum += $program_tree['holding'][$id]['tree_weight'];
            //$program_tree['holding'][$id]['tree_weight'] = $sum += $tree['weight'];
        }
        $program_tree['tree_weight'] = $sum + $program_tree['weight'];
    } else {
        $program_tree['tree_weight'] = $program_tree['weight'];
    }
}

function findBrokenBranch($program_tree){
    if(isset($program_tree['holding']) && count($program_tree['holding'])){
        $weights = [];
        foreach($program_tree['holding'] as $id=>$tree){
             $weights[$id] = $tree['tree_weight'];
        }
        $w = [];
        ...
        $unique_weight = array_search(1,$w);
        if($unique_weight !== false){
            $id = array_search($unique_weight,$weights);
            if(isset($program_tree['holding'][$id]['holding']) && count($program_tree['holding'][$id]['holding'])){
                $broken_tree = $program_tree['holding'][$id];
                $broken_node = findBrokenBranch($broken_tree);
                if(!$broken_node){
                    foreach($weights as $weight){
                        if($weight != $unique_weight){
                            $diff = $weight - $unique_weight;
                        }
                    }
                    die($broken_tree['weight'] + $diff . '');
                }
            } else {
                foreach($weights as $weight){
                    if($weight != $unique_weight){
