...
import org.singlespaced.d3js.scale._
...
object TutorialApp extends js.JSApp {
  ...
  def main(): Unit = {
    ...
    val svg1 = d3.select("body").append("svg")
              .attr("width", 900)
              .attr("height", 300)

    val color = d3.scale.category20

    val simulation = d3.layout.force()
        simulation.gravity(0.05)
        simulation.linkDistance(100)
        simulation.charge(-100)
        simulation.size(js.Tuple2(900, 300))
